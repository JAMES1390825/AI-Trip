...
func TestHandleChatIntakeNextUsesAIEnhancement(t *testing.T) {
	aiServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/chat/enhance" {
			t.Fatalf("unexpected ai path %s", r.URL.Path)
		}
		writeJSON(w, http.StatusOK, map[string]any{
			"assistant_message": "我先帮你把已确认的信息收好，接下来只差出发日期。",
			"suggested_options": []string{"2026-04-10", "2026-04-17"},
	...
	body, err := json.Marshal(map[string]any{
		"history": []map[string]any{
			{"role": "user", "message": "我想去杭州玩三天"},
		},
		"draft_plan_request": map[string]any{
			"origin_city":  "上海",
			"destination":  "杭州",
			"days":         3,
			"budget_level": "medium",
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/chat/intake/next", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rr := httptest.NewRecorder()

	app.handleChatIntakeNext(rr, req, &AuthUser{UserID: "u-1"})
	...
	if asString(resp["assistant_message"]) != "我先帮你把已确认的信息收好，接下来只差出发日期。" {
		t.Fatalf("expected ai assistant message, got %q", asString(resp["assistant_message"]))
	}
	if asString(resp["source_mode"]) != "llm_bailian" {
		t.Fatalf("expected source_mode llm_bailian, got %q", asString(resp["source_mode"]))
	}
	options := asStringSlice(resp["suggested_options"])
	if len(options) != 2 {
		t.Fatalf("expected 2 suggested options, got %#v", options)
	}
}
