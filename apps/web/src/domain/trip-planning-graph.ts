...
const GraphState = Annotation.Root({
  ...
  queries: Annotation<AmapPoiQuery[]>({ reducer: (_, next) => next, default: () => [] }),
  candidates: Annotation<RealPoiCandidate[]>({ reducer: (_, next) => next, default: () => [] }),
  evidenceSources: Annotation<EvidenceSources>({ reducer: (_, next) => next, default: () => [] }),
  ...
  repairAttempts: Annotation<number>({ reducer: (_, next) => next, default: () => 0 }),
  stops: Annotation<RouteStop[]>({ reducer: (_, next) => next, default: () => [] }),
  legs: Annotation<RouteLeg[]>({ reducer: (_, next) => next, default: () => [] }),
  routeCard: Annotation<RouteCard | undefined>,
  planningTrace: Annotation<PlanningTraceEvent[]>({ reducer: (_, next) => next, default: createQueuedPlanningTrace }),
